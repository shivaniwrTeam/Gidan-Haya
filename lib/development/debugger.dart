...
class Debugger {
  ...
  static bool releaseTracking = true;
  static void clearRequest() async {
    if (!kDebugMode && releaseTracking == false) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ...
        MaterialButton(
          onPressed: () {
            if (Hive.box(widget.boxKey).get(selectedKey) is int) {
              Hive.box(widget.boxKey)
                  .put(selectedKey, int.parse(_valueController.text));
            } else {
      ...
      content: Column(
        ...
        children: [
          ...
          } else if (Hive.box(widget.boxKey).get(selectedKey) is int) ...{
            TextField(
