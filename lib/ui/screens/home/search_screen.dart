...
class SearchScreenState extends State<SearchScreen>
  ...
  void initState() {
    ...
    context.read<SearchPropertyCubit>().searchProperty('',
        ...
        callInfo:
            CallInfo(from: 'init|search property', fromFile: 'search screen'));
    ...
    if (controller.isEndReached()) {
      if (context.read<SearchPropertyCubit>().hasMoreData()) {
        context.read<SearchPropertyCubit>().fetchMoreSearchData(
            callInfo: CallInfo(
                from: 'listner|search |more', fromFile: 'search screen'));
      }
    }
  }
  ...
  void propertySearch() {
    ...
    if (previouseSearchQuery != searchController.text) {
      context.read<SearchPropertyCubit>().searchProperty(searchController.text,
          ...
          filter: selectedFilter,
          callInfo:
              CallInfo(from: 'property search', fromFile: 'search screen'));
          ...
          Navigator.pushNamed(context, Routes.filterScreen).then((value) {
            if (value != null && searchController.text != '') {
              ...
              context.read<SearchPropertyCubit>().searchProperty(
                  ...
                  filter: value,
                  callInfo: CallInfo(
                      from: 'on tap filter button', fromFile: 'search screen'));
            }
          });
        },
        icon: Icon(
        ...
        return NoInternet(
          onRetry: () {
            context.read<SearchPropertyCubit>().searchProperty('',
                ...
                filter: selectedFilter,
                callInfo:
                    CallInfo(from: 'no internet', fromFile: 'search screen'));
          },
        );
      }
                  ...
                  Navigator.pushNamed(context, Routes.filterScreen,
                    ...
                    if (value != null) {
                      ...
                      context.read<SearchPropertyCubit>().searchProperty(
                          ...
                          filter: value,
                          callInfo: CallInfo(
                              from: 'filter screen back value',
                              fromFile: 'search screen'));
                      setState(() {});
