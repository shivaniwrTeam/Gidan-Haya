...
class LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
      ...
      if (state is MSuccess) {
        ...
        if (widget.isDeleteAccount ?? false) {
          context.read<DeleteAccountCubit>().deleteUserAccount(context,
              callInfo:
                  CallInfo(from: 'delete account', fromFile: 'login_screen'));
        } else {
          context.read<LoginCubit>().login(
                ...
                callInfo: CallInfo(from: 'login', fromFile: 'login_screen'),
              );
        }
      }
    ...
    context.read<FetchSystemSettingsCubit>().fetchSettings(
          isAnonymouse: true,
          forceRefresh: true,
          callInfo: CallInfo(from: 'init|settings', fromFile: 'login_screen'),
        );
                ...
                listener: (context, state) async {
                  ...
                  if (state is LoginSuccess) {
                    ...
                    context.read<FetchSystemSettingsCubit>().fetchSettings(
                        ...
                        forceRefresh: true,
                        callInfo: CallInfo(
                            from: 'login success|settings',
                            fromFile: 'login_screen'));
                    ...
                    if (state.isProfileCompleted) {
                      ...
                      context.read<AuthCubit>().updateFCM(context,
                          callInfo: CallInfo(
                              from: 'login success|profie complete',
                              fromFile: 'login_screen'));
                      if (widget.popToCurrent == true) {
                        Navigator.pop(context);
                      } else {
                      ...
                      context.read<AuthCubit>().updateFCM(context,
                          callInfo: CallInfo(
                              from: 'login success|profie not complete',
                              fromFile: 'login_screen'));
                      ...
                      if (state is VerifyOtpSuccess) {
                        if (widget.isDeleteAccount ?? false) {
                          context.read<DeleteAccountCubit>().deleteUserAccount(
                              context,
                              callInfo: CallInfo(
                                  from: 'verfify otp success|delete account',
                                  fromFile: 'login_scren'));
                        } else {
                          context.read<LoginCubit>().login(
