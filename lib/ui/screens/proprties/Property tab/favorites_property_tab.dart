...
class _FavoritesScreenState extends State<FavoritesScreen>
  ...
  void initState() {
    ...
    context.read<FetchFavoritesCubit>().fetchFavorites(
        callInfo: CallInfo(from: 'init state', fromFile: 'favorites'));
    super.initState();
  }
    ...
    if (_pageScrollController.isEndReached()) {
      if (context.read<FetchFavoritesCubit>().hasMoreData()) {
        context.read<FetchFavoritesCubit>().fetchFavoritesMore(
            callInfo: CallInfo(from: 'listener|more', fromFile: 'favorites'));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      onRefresh: () async {
        context.read<FetchFavoritesCubit>().fetchFavorites(
            callInfo: CallInfo(from: 'refresh', fromFile: 'favorites'));
      },
      child: Scaffold(
            ...
            if (state is FetchFavoritesSuccess) {
              if (state.propertymodel.isEmpty) {
                return NoDataFound(
                  onTap: () {
                    context.read<FetchFavoritesCubit>().fetchFavorites(
                        callInfo:
                            CallInfo(from: 'no-data', fromFile: 'favorites'));
                  },
                );
              }
              ...
              return RefreshIndicator(
                ...
                onRefresh: () async {
                  context.read<FetchFavoritesCubit>().fetchFavorites(
                      callInfo:
                          CallInfo(from: 'refresh', fromFile: 'favorites'));
                },
                child: Column(
                  children: [
                    Expanded(
