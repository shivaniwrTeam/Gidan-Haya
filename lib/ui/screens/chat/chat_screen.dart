...
class _ChatScreenState extends State<ChatScreen>
        ...
        if (_pageScrollController.offset >=
          ...
          if (context.read<LoadChatMessagesCubit>().hasMoreChat()) {
            context.read<LoadChatMessagesCubit>().loadMore(
                callInfo: CallInfo(
                    from: 'listener|more chat', fromFile: 'chat_Screen'));
          }
        }
      },
    );
  ...
  void initState() {
    ...
    context.read<LoadChatMessagesCubit>().load(
          ...
          callInfo: CallInfo(from: 'init|chat load', fromFile: 'chat_screen'),
        );
  ...
  Widget build(BuildContext context) {
    ...
    // return Container();
    print("widget.profilePicture :${widget.profilePicture.isEmpty}");
    return WillPopScope(
      onWillPop: () async {
                      ...
                      try {
                        ...
                        DataOutput<PropertyModel> dataOutput = await fetch
                            .fetchPropertyFromPropertyId(widget.propertyId,
                                callInfo: CallInfo(
                                    from:
                                        'on tap property image|property from id',
                                    fromFile: 'chat_Screen'));
                        Future.delayed(
                          Duration.zero,
                          () {
...
Widget getDateChip(String date) {
  return Padding(
    padding: const EdgeInsets.all(10.0),
    child: Text(date.toString()).size(15),
  );
}
                        ...
                        try {
                          ...
                          DataOutput<PropertyModel> dataOutput = await fetch
                              .fetchPropertyFromPropertyId(propertyId,
                                  callInfo: CallInfo(
                                      from:
                                          'view property|fetch property from id',
                                      fromFile: 'chat_Screen'));
                          Future.delayed(
                            Duration.zero,
                            () {
                          ...
                          buttonTitle:
                              UiUtils.translate(context, 'viewProperty'),
