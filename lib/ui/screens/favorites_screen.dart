...
class _FavoritesScreenState extends State<FavoritesScreen> {
  ...
  void initState() {
    ...
    context.read<FetchFavoritesCubit>().fetchFavorites(
        callInfo:
            CallInfo(from: 'Init state', fromFile: 'favorites_screen.dart'));
    super.initState();
  }
    ...
    if (_pageScrollController.isEndReached()) {
      if (context.read<FetchFavoritesCubit>().hasMoreData()) {
        context.read<FetchFavoritesCubit>().fetchFavoritesMore(
            callInfo:
                CallInfo(from: 'Listener', fromFile: 'favorites_screen.dart'));
      }
    }
  }
                ...
                return NoInternet(
                  onRetry: () {
                    context
                        ...
                        .fetchFavorites(callInfo: CallInfo(from: ''));
                  },
                );
              }
                      ...
                      child: NoDataFound(
                        onTap: () {
                          context.read<FetchFavoritesCubit>().fetchFavorites(
                              callInfo: CallInfo(
                                  from: 'no data found',
                                  fromFile: 'favorites_screen.dart'));
                        },
                      ),
                    ),
                  ),
