...
class SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    context.read<FetchCategoryCubit>().fetchCategories(
        callInfo: CallInfo(from: 'Init state', fromFile: 'splash_screen.dart'));
    ...
    Connectivity().checkConnectivity().then((value) {
      if (value.contains(ConnectivityResult.none) && !isDataAvailable) {
    ...
    Future.delayed(Duration.zero, () {
      context.read<ProfileSettingCubit>().fetchProfileSetting(
            ...
            Api.currencySymbol,
            callInfo:
                CallInfo(from: 'init state', fromFile: 'splash_screen.dart'),
          );
    });
  }
  ...
  void checkIsUserAuthenticated() async {
    ...
    print('Is authenticated ${authenticationState}');
    if (authenticationState == AuthenticationState.authenticated) {
      ...
      context.read<FetchSystemSettingsCubit>().fetchSettings(
            isAnonymouse: false,
            callInfo: CallInfo(
                from: 'If authenticated', fromFile: 'splash_screen.dart'),
          );
      ...
      context.read<FetchSystemSettingsCubit>().fetchSettings(
          isAnonymouse: true,
          callInfo: CallInfo(
              from: 'If not authenticated', fromFile: 'splash_screen.dart'));
    }
  }
...
Future getDefaultLanguage(VoidCallback onSuccess,
  ...
  try {
    ...
    if (HiveUtils.getLanguage() == null ||
      ...
      Map result = await SystemRepository().fetchSystemSettings(
        isAnonymouse: true,
        callInfo: CallInfo(
            from: 'if language data is null', fromFile: 'splash_screen.dart'),
      );
