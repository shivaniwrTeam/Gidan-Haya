      ...
      if (_scrollController.isEndReached()) {
        if (context.read<FetchSubscriptionPackagesCubit>().hasMore()) {
          context.read<FetchSubscriptionPackagesCubit>().fetchMorePackages(
              callInfo: CallInfo(
                  from: 'More packages', fromFile: 'packages_list.dart'));
        }
      }
    });
  ...
  Future<void> _onTapSubscribe(subscriptionPackage) async {
    ...
    if (subscriptionPackage.price?.toInt() == 0) {
      context.read<AssignFreePackageCubit>().assign(subscriptionPackage.id!,
          callInfo: CallInfo(
              from: 'free package subscribe', fromFile: 'package_list'));
      return;
    }
    if (Platform.isIOS) {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      onRefresh: () async {
        context.read<FetchSubscriptionPackagesCubit>().fetchPackages(
            callInfo:
                CallInfo(from: 'On refresh', fromFile: 'package_list.dart'));
      },
      child: Scaffold(
        backgroundColor: context.color.primaryColor,
                ...
                listener: (context, state) {
                  if (state is AssignInAppPackageSuccess) {
                    context.read<FetchSystemSettingsCubit>().fetchSettings(
                        ...
                        forceRefresh: true,
                        callInfo: CallInfo(
                            from: 'if packages assign success',
                            fromFile: 'packages_list'));
                ...
                listener: (context, state) {
                  ...
                  if (state is AssignFreePackageSuccess) {
                    ...
                    context
                        ...
                        .fetchPackages(
                            callInfo: CallInfo(
                                from: 'if free packages assign success',
                                fromFile: 'packages_list'));
                    context.read<FetchSystemSettingsCubit>().fetchSettings(
                        isAnonymouse: false,
                        forceRefresh: true,
          ...
          Padding(
            ...
            child: UiUtils.buildButton(context, onPressed: () async {
              if (subscriptionPacakge.price?.toInt() == 0) {
                context.read<AssignFreePackageCubit>().assign(
                    subscriptionPacakge.id!,
                    callInfo: CallInfo(
                        from: 'on assign free pacakge',
                        fromFile: 'packages_list'));
                return;
              }
              if (Platform.isIOS) {
