...
class _PropertyMapScreenState extends State<PropertyMapScreen> {
          ...
          if (previouseSearchQuery != _searchController.text) {
            try {
              ...
              cities = await GooglePlaceRepository().serchCities(
                ...
                callInfo: CallInfo(
                    from: 'search cities',
                    fromFile: 'property_map_screen.dart'),
              );
  ...
  Future<void> loadDefaultCity() async {
    ...
    List<MapPoint> pointList = await GMap.getNearByProperty(
        ...
        callInfo: CallInfo(
            from: 'get nearby property', fromFile: 'property_map+screen.dart'));
  ...
  Future<void> onTapCity(int index) async {
    ...
    List<MapPoint> pointList = await GMap.getNearByProperty(
        ...
        callInfo: CallInfo(
            from: 'nearby Property', fromFile: 'property_map_screen.dart'));
  ...
  void loopMarker(List<MapPoint> pointList) {
    for (var i = 0; i < pointList.length; i++) {
      ...
      marker
          .addLabelMarker(LabelMarker(
        label: Constant.currencySymbol +
            (element.price).toString().priceFormate(context: context),
        markerId: MarkerId('$i'),
        onTap: () async {
          selectedMarker = i;
  ...
  Future<void> fetchProperty(int id) async {
    try {
      ...
      DataOutput<PropertyModel> result = await PropertyRepository()
          .fetchPropertyFromPropertyId(id,
              callInfo: CallInfo(
                  from: 'property map fetch property',
                  fromFile: 'property_map_screen.dart'));
  ...
  Future<LatLng> getCityLatLong(String placeId) async {
    var rawCityLatLong = await GooglePlaceRepository()
        .getPlaceDetailsFromPlaceId(placeId,
            callInfo: CallInfo(
                from: 'city lat long', fromFile: 'property_map_screen'));
