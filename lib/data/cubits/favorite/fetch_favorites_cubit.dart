...
class FetchFavoritesCubit extends Cubit<FetchFavoritesState> {
  ...
  Future<void> fetchFavorites({required CallInfo callInfo}) async {
    try {
      emit(FetchFavoritesInProgress());

      DataOutput<PropertyModel> result = await _favoritesRepository
          .fechFavorites(offset: 0, callInfo: callInfo);
  ...
  Future<void> fetchFavoritesMore({required CallInfo callInfo}) async {
    try {
      if (state is FetchFavoritesSuccess) {
