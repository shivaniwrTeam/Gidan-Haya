...
class GetInterestedUserCubit extends Cubit<GetInterestedUserState> {
  ...
  void fetch(String propertyId, {required CallInfo callInfo}) async {
    try {
      emit(GetInterestedUserInProgress());
      DataOutput<InterestedUserModel> result = await _interestRepository
          .getInterestUser(propertyId, offset: 0, callInfo: callInfo);
  ...
  fetchMore({required CallInfo callInfo}) async {
    try {
