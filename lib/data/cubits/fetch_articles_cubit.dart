...
class FetchArticlesCubit extends Cubit<FetchArticlesState> {
  ...
  Future<void> fetchArticles({required CallInfo callInfo}) async {
    try {
      emit(FetchArticlesInProgress());

      DataOutput<ArticleModel> result =
          await _articleRepository.fetchArticles(offset: 0, callInfo: callInfo);
