...
class FetchCategoryCubit extends Cubit<FetchCategoryState> with HydratedMixin {
    ...
    try {
      await CacheData().getData<FetchCategorySuccess>(
          ...
          onNetworkRequest: () async {
            DataOutput<Category> categories = await _categoryRepository
                .fetchCategories(offset: 0, callInfo: callInfo);
  ...
  Future<Category> get(int id, {required CallInfo callInfo}) async {
    try {
      DataOutput<Category> dataOutput = await _categoryRepository
          .fetchCategories(offset: 0, id: id, callInfo: callInfo);
  ...
  Future<void> fetchCategoriesMore({required CallInfo callInfo}) async {
    try {
      if (state is FetchCategorySuccess) {
