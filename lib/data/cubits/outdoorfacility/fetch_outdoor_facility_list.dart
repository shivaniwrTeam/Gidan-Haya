...
class FetchOutdoorFacilityListCubit
  ...
  void fetch({required CallInfo callInfo}) async {
    try {
      ...
      List<OutdoorFacility> facilityList = await _facilityRepository
          .fetchOutdoorFacilityList(callInfo: callInfo);
  ...
  void fetchIfFailed() {
    if (state is FetchOutdoorFacilityListFailure ||
      ...
      fetch(
          callInfo: CallInfo(
              from: 'If failed then fetch',
              fromFile: 'fecth_outdoor_facility_list.dart'));
    }
  }
