...
class FetchProjectsCubit extends Cubit<FetchProjectsState> {
  ...
  Future<void> fetchProjects({required CallInfo callInfo}) async {
    try {
      emit(FetchProjectsInProgress());
      DataOutput<ProjectModel> result =
          await _projectRepository.getProjects(offset: 0, callInfo: callInfo);
  ...
  fetchMoreProjects({required CallInfo callInfo}) async {
