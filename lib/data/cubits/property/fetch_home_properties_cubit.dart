...
class FetchHomePropertiesCubit extends Cubit<FetchHomePropertiesState> {
  ...
  void fetchProperty({required CallInfo callInfo}) async {
    try {
      emit(FetchHomePropertiesInProgress());
      DataOutput<PropertyModel> result = await _propertyRepository
          .fetchProperty(offset: 0, callInfo: callInfo);
  ...
  Future<void> fetchMoreProperty({required CallInfo callInfo}) async {
    try {
