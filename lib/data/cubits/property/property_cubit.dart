...
class PropertyCubit extends Cubit<PropertyState> {
  ...
  void fetchProperty(
    ...
    fetchPropertyFromDb(context, mbodyparam,
            fromUserlist: fromUserlist, callInfo: callInfo)
        .then((value) {
  ...
  Future<Map> fetchPropertyFromDb(
      ...
      {bool fromUserlist = false, required CallInfo callInfo}) async {
    //String? propertyId,
    Map result = {};
    List<PropertyModel> propertylist = [];
    int mtotal = 0;
    var response = await Api.post(
        url: Api.apiGetProprty, callInfo: callInfo, parameter: {});
    // log("server data $map");
