...
class SliderCubit extends Cubit<SliderState> with HydratedMixin {
  ...
  void fetchSlider(BuildContext context,
      {bool? forceRefresh, bool? loadWithoutDelay,required CallInfo callInfo}) async {
    ...
    if (forceRefresh == true) {
      fetchSliderFromDb(context, sendCityName: true,callInfo:callInfo )
  ...
  Future<List<HomeSlider>> fetchSliderFromDb(BuildContext context,
      {required bool sendCityName, required CallInfo callInfo}) async {
    List<HomeSlider> sliderlist = [];
    ...
    var response = await Api.get(
        url: Api.apiGetSlider, queryParameters: body, callInfo: callInfo);
