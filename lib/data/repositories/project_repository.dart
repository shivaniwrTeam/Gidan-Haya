...
class ProjectRepository {
  createProject(Map projectPayload, {required CallInfo callInfo}) async {
    try {
      ...
      Map<String, dynamic> map = await Api.post(
          url: Api.postProject, parameter: multipartedData, callInfo: callInfo);
      return map;
    } catch (e, st) {
      // throw e;
    }
  ...
  Future<DataOutput<ProjectModel>> getProjects(
      {int? offset, required CallInfo callInfo}) async {
    Map<String, dynamic> result = await Api.get(
        url: Api.getProjects,
